...
需要先说明的是：这并不是一个“产品”，只是我根据个人需求搭的一个简单工作框架。把它公开出来，更多是作为经验和想法的分享——如果觉得有用，按需取用、自行调整即可。
...
这个步骤我想对于大多数人是多余的，因为使用的都是最基础的包。当然，也可以按自己的偏好调整（比如锁定版本）。
...
[`example_task/`](example_task/Memo.md) 是一个我自己真实跑过的任务（某一周的市场快照）。当时用的模型是 `glm-5.2`，harness 是 pi。具体用的提示词忘了，大概就是让它做一个快照。仅作参考，开始跑自己的任务之前把它删掉即可。

## 一些想法

我个人推荐将 skill 安装在这个工作目录内，因为在我们的使用场景里，几乎所有涉及 Tushare 的任务都会在这个目录下运行。而且我认为，我们应该把这样一个工作目录视为一个单元来管理。
...
Skills 的核心在于 progressive disclosure，也就是说，如果某些信息几乎在所有任务中都会用到，那么与其专门写 skills，或许不如直接把它放到 AGENTS.md 里。此外，请不要低估手写 SKILL 或 AGENTS.md 的价值。如果你不想自己的 agent 只能沦为华而不实的玩具，那么静下心写这些文件是绝对有必要且有意义的（当然我们可以借助 AI 辅助来写，但其中必须要有我们的观点）。
